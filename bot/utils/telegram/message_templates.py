...
async def generate_cleaning_messages(bot: Bot, chat_id: int, cleaning_result: dict | None) -> list[str]:
    ...
    cleaning_availibility = await check_cleaning_accuracy(chat_id)
    warning = "" if cleaning_availibility else "\n<i>ℹ️ Бот в чате недавно, статистика может быть неполной.</i>"

    if not cleaning_result or cleaning_result.get("data", {}).get("total_users", 0) == 0:
        return [f"✅ Все участники активны или находятся в ресте.{warning}"]

    users = cleaning_result["users"]
    ...
    # --- Хелпер для добавления строк с пагинацией ---
    def append_line(line: str, inside_quote: bool = False):
        nonlocal current_text
        if len(current_text) + len(line) > MAX_MESSAGE_LENGTH:
            if inside_quote:
                current_text += "</blockquote>"
            messages_list.append(current_text)
            current_text = "<blockquote expandable>" if inside_quote else ""
            current_text += line
        else:
            current_text += line
    ...
    # Разносим юзеров по спискам на основе флагов
    # ---------------------------------------------------------
    
    # 1. Призраки (вообще нет сообщений). Это самый строгий критерий.
    ghosts = [u for u in users if u.get("no_activity")]

    # 2. Неактивные (давно не писали). Исключаем призраков, чтобы не дублировать.
    sleepers = [u for u in users if u.get("inactivity") and not u.get("no_activity")]

    # 3. Малоактивные (мало сообщений). 
    # Исключаем призраков (у них и так 0), Но ОСТАВЛЯЕМ sleepers, т.к нам нужны все флаги
    low_activity = [u for u in users if u.get("low_activity") and not u.get("no_activity")]

    # Сортируем списки
    # Sleepers: от самых давних к недавним
    sleepers.sort(key=lambda x: x["last_message_date"] or now) 
    # Low activity: от меньшего кол-ва сообщений к большему
    low_activity.sort(key=lambda x: x["message_count"] or 0)

    # ---------------------------------------------------------
    # БЛОК 1: 📛 НИКОГДА НЕ ПИСАЛИ (Ghost / No Activity)
    # ---------------------------------------------------------
    if ghosts:
        header = f"📛 Никогда не писали ({len(ghosts)}):\n"
        append_line(header)
        append_line("<blockquote expandable>", inside_quote=False)
        
        for u in ghosts:
            mention = await mention_user_with_delay(bot, chat_id, int(u["user_id"]))
            line = f"▫️ {mention}\n"
            append_line(line, inside_quote=True)
            
        append_line("</blockquote>\n\n")

    # ---------------------------------------------------------
    # БЛОК 2: 💤 ДАВНО МОЛЧАТ (Inactivity)
    # ---------------------------------------------------------
    if sleepers:
        cutoff_duration = now - data["inactive_cutoff"]
        duration_str = TimedeltaFormatter.format(cutoff_duration, suffix='none')
        
        header = f"💤 Неактив более {duration_str}:\n"
        append_line(header)
        append_line("<blockquote expandable>")

        for u in sleepers:
            mention = await mention_user_with_delay(bot, chat_id, int(u["user_id"]))
            
            # Вычисляем сколько молчит
            last_msg = u["last_message_date"]
            if last_msg:
                delta = now - last_msg
                time_str = TimedeltaFormatter.format(delta, suffix="none")
            else:
                time_str = "???" # Should not happen given logic above

            line = f"▫️ {mention}: {time_str}\n"
            append_line(line, inside_quote=True)

        append_line("</blockquote>\n\n")

    # ---------------------------------------------------------
    # БЛОК 3: ⚠️ МАЛО АКТИВА (Low Activity)
    # ---------------------------------------------------------
    if low_activity:
        header = f"⚠️ Нету нормы ({data['min_messages']} сообщ.):\n"
        append_line(header)
        append_line("<blockquote expandable>")

        for u in low_activity:
            mention = await mention_user_with_delay(bot, chat_id, int(u["user_id"]))
            count = u['message_count']
            percentage = (count / data["min_messages"]) * 100

            line = f"▫️ {mention}: <b>{count}</b> ({percentage:.0f}%)\n"
            append_line(line, inside_quote=True)

        append_line("</blockquote>\n\n")
    
    append_line(f"🧹 Всего {len(users)} чел.{warning}\n\n")
