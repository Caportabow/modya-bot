...
async def show_rp_commands_handler(msg: Message):
    ...
    for command, template in commands.items():
        ...
        ans += f"{command} {emoji} —> {action}\n"
    ans += "</blockquote>\n\n"
    ans += f"Всего: {len(commands)}"
...
async def export_rp_commands_handler(msg: Message):
    ...
    if export_id is not None:
        ...
        if added_cmds:
            ...
            await msg.reply(
                f"✅ Добавлено {len(added_cmds)} команды:\n\n"
                f"<code>{cmd_list}</code>{more}",
                parse_mode="HTML"
            )
        else:
            await msg.reply(
